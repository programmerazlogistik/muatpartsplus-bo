...
export const transformTruckTypesToTableData = (apiData) => {
  ...
  return apiData.map((truckType, index) => ({
    id: truckType.id,
    description: truckType.description,
...
export const filterTruckTypesByCategory = (truckTypesData, category) => {
  if (!truckTypesData || !Array.isArray(truckTypesData) || !category) return truckTypesData;
  
  return truckTypesData.filter(truckType => 
    getVehicleCategory(truckType.description) === category
  );
};
...
export const searchTruckTypesByDescription = (truckTypesData, searchTerm) => {
  if (!truckTypesData || !Array.isArray(truckTypesData) || !searchTerm) return truckTypesData;
...
export const getTruckTypeById = (truckTypesData, truckTypeId) => {
  if (!truckTypesData || !Array.isArray(truckTypesData) || !truckTypeId) return null;
  
  return truckTypesData.find(truckType => truckType.id === truckTypeId) || null;
};
...
export const getTruckTypesSummary = (truckTypesData) => {
  ...
  const mostCommonCategory = Object.keys(stats.byCategory).reduce((a, b) => 
    stats.byCategory[a] > stats.byCategory[b] ? a : b, null
  );
...
export const validateTruckTypeData = (truckTypeData) => {
  ...
  if (!truckTypeData.description || truckTypeData.description.trim() === '') {
    ...
    errors.description = "Truck Type description must be less than 100 characters";
  }
...
export const checkTruckTypeExists = (truckTypesData, truckTypeId) => {
  if (!truckTypesData || !Array.isArray(truckTypesData) || !truckTypeId) return false;
...
export const getTruckTypeByDescription = (truckTypesData, description) => {
  if (!truckTypesData || !Array.isArray(truckTypesData) || !description) return null;
  
  return truckTypesData.find(truckType => 
