/**
 * @typedef {import('./filter-dropdown.d.ts').FilterDropdownProps} FilterDropdownProps
 */
...
/**
 * A powerful multi-category filter dropdown component with search capabilities,
 * multi-select support, and custom trigger options.
 *
 * @param {FilterDropdownProps} props - Component props
 * @returns {React.ReactElement}
 */
const FilterDropdownImplementation = ({
  trigger,
  categories = [],
  data = {},
  selectedValues = {},
  ...
  useEffect(() => {
    return () => {
      Object.values(hoverTimeouts.current).forEach((timeout: number) =>
                        ...
                        >
                          {multiSelect ? (
                            ...
                            <RadioButton
                              ...
                              value={item.id}
                              label={item.label}
                              classNameLabel="text-xs font-medium line-clamp-2 break-all"
                            />
                          )}
                        </div>
                      ))
...
FilterDropdownImplementation.displayName = "FilterDropdown";

export default FilterDropdownImplementation;
