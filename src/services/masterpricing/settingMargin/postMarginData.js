...
export const transformPostMarginRequest = (formData) => {
  ...
  if (!margin || typeof margin !== 'number' || margin < 0) {
    throw new Error("Margin is required and must be a number greater than or equal to 0");
  }
...
export const validateMarginCreationData = (data) => {
  ...
  if (!data.margin || typeof data.margin !== 'number' || data.margin < 0) {
    errors.margin = "Margin wajib diisi dan harus lebih dari atau sama dengan 0";
  } else if (data.margin > 100) {
...
export const sanitizeMarginPercentage = (margin) => {
  if (!margin || typeof margin !== 'number') return 0;
